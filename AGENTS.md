...
## 项目概览
RMCS（RoboMaster Control System）是基于 ROS 2 Jazzy 的机器人控制系统。开发在 Docker devcontainer 中进行（镜像 `qzhhhi/rmcs-develop`），部署到 MiniPC 上的 runtime 容器。当前分支 `dev/sp-vision` 正在集成 sp_vision_25 的 Planner 到 omni-infantry 的云台控制链。
...
### 运行
- `set-robot omni-infantry`：写入 `RMCS_ROBOT_TYPE` 到 `~/env_setup.{bash,zsh}`
...
## rmcs_core 全部组件一览（28 个 pluginlib 插件）

| 分类 | 组件 |
|------|------|
| 硬件 | `MecanumHero`, `SteeringHero`, `OmniInfantry`, `SteeringInfantry` |
| 底盘控制 | `ChassisController`, `ChassisPowerController`, `OmniWheelController`, `SteeringWheelController` |
| 云台控制 | `SimpleGimbalController`, `OmniInfantryPlannerGimbalController`, `HeroGimbalController`, `DualYawController`, `PlayerViewer` |
| 射击控制 | `FrictionWheelController`, `HeatController`, `BulletFeederController17mm`, `BulletFeederController42mm`, `ShootingRecorder` |
| PID | `PidController`, `ErrorPidController` |
| 广播 | `TfBroadcaster`, `ValueBroadcaster` |
| 裁判系统 | `Status`, `Command`, `Interaction`, `Ui`, `Infantry`(ui), `Hero`(ui) |

sp_vision_25 另注册 1 个组件：`SpVisionBridge`。
