...
struct Head {
    ...
    // 令其值正确
    void MakeCorrect() { data = HeadValue; }

    // 令其值不正确
    void MakeIncorrect() { data = ~HeadValue; }

    // 判断值是否正确
    bool IsCorrect() { return data == HeadValue; }
};
...
class SerialSender {
public:
    explicit SerialSender(serial::Serial& serial)
        : _serial(serial) {
...
public:
    ...
    void Send() { _serial.write(reinterpret_cast<uint8_t*>(&_pkg), sizeof(_pkg)); }
...
public:
    explicit SerialReceiver(serial::Serial& serial)
        : _serial(serial) {}
    ...
    ReceiveResult Receive() {
        ...
        // 接收尽可能多的数据
        _cacheSize += _serial.read(
            reinterpret_cast<uint8_t*>(&_receivePkg->head) + _cacheSize,
            sizeof(PackageType) - _cacheSize);

        if (_cacheSize >= sizeof(HeadType)) {    // 成功接收到包头部分
            if (_receivePkg->head.IsCorrect()) { // 数据包头正确
                if (_cacheSize
                    == sizeof(PackageType)) { // 若包头正确且数据接收完整，则尝试进一步校验
                    ...
                    return ReceiveResult::
                        Timeout; // 若包头正确但数据未接收完整，则返回Timeout，等待下一次接收
            } else
                result = ReceiveResult::InvaildHeader;
            ...
            while (true) {
                if (_cacheSize < sizeof(HeadType)
                    || reinterpret_cast<HeadType*>(bufferHead)->IsCorrect()) {
            ...
            return ReceiveResult::Timeout; // 若连包头都没接到，也返回Timeout
    }
...
public:
    ...
    ReceiveResult Receive() {
        _cacheSize += _serial.read(
            reinterpret_cast<uint8_t*>(_receivePkg) + _cacheSize, sizeof(DataType) - _cacheSize);
...
public:
    ...
    ReceiveResult Receive() {
        ...
        _cacheSize += _serial.read(
            reinterpret_cast<uint8_t*>(&_receivePkg->head) + _cacheSize,
            sizeof(HeadType) + sizeof(DataType) - _cacheSize);

        if (_cacheSize >= sizeof(HeadType)) {    // 成功接收到包头部分
            if (_receivePkg->head.IsCorrect()) { // 数据包头正确
                if (_cacheSize
                    == sizeof(HeadType)
                           + sizeof(DataType)) { // 若包头正确且数据接收完整，则返回Success
                    _cacheSize = 0;
                    std::swap(_receivePkg, _resultPkg);
                    return ReceiveResult::Success;
                } else
...
public:
    ...
    ReceiveResult Receive() {
        // 接收尽可能多的数据
        _cacheSize += _serial.read(
            reinterpret_cast<uint8_t*>(&_receivePkg->data) + _cacheSize,
            sizeof(DataType) + sizeof(typename ChecksumCalculatorType::ResultType) - _cacheSize);

        if (_cacheSize
            == sizeof(DataType)
                   + sizeof(typename ChecksumCalculatorType::
                                ResultType)) { // 若数据接收完整，则尝试进一步校验
            if (ChecksumCalculatorType::Verify(*_receivePkg)) {
                // 校验通过则返回Success
                _cacheSize = 0;
                std::swap(_receivePkg, _resultPkg);
...
private:
    ...
    // _receivePkg指向接收缓冲区, _resultPkg指向上一个已接收并验证通过的缓冲区
    Package<DataType, None, typename ChecksumCalculatorType::ResultType> _pkg[2],
        *_receivePkg = &_pkg[0], *_resultPkg = &_pkg[1];
};
