...
public:
    ...
    void insert(Values value, Weight weight) {
        if (buffer.insert(value, weight) == TDigestImpl::insert_result::NEED_COMPRESS) {
            merge();
        }
    }
...
void TDigest<Values, Weight>::merge() {
    ...
    if (run_forward) {
        ...
        max_val = std::max(
            max_val,
            inputs.back().weight == 1 ? inputs.back().mean : std::numeric_limits<Values>::lowest());
        ...
        min_val = std::min(
            min_val,
            inputs.back().weight == 1 ? inputs.back().mean : std::numeric_limits<Values>::max());

        max_val = std::max(
            max_val, inputs.front().weight == 1 ? inputs.front().mean
...
double TDigest<Values, Weight>::quantile(double p) const {
    ...
    if (last.weight > 1 && active->total_weight - index <= last.weight / 2) {
        return (
            max_val
            - static_cast<double>(active->total_weight - index - 1) / (last.weight / 2 - 1)
                  * (max_val - last.mean));
    }
...
double TDigest<Values, Weight>::cumulative_distribution(Values x) const {
    ...
    if (x > active->values.back().mean) {
        ...
        if (max_val - last.mean > 0) {
            return (
                1
                - (lerp(1, last.weight / 2 - 1, (max_val - x) / (max_val - last.mean))
                   / active->total_weight));
        }
        return (1.0);
    }
