 ...
 *   https://github.com/SpirentOrion/digestible/blob/master/include/digestible/digestible.h
 *
 * The digestible project provides a modern C++ implementation of a merging
 * t-digest data structure under the Apache License, Version 2.0:
 *   https://github.com/SpirentOrion/digestible
 *
 * The underlying t-digest algorithm and reference implementation were originally
 * authored by Ted Dunning:
 *   https://github.com/tdunning/t-digest
 *
 * As noted in the original t-digest NOTICES file:
 ...
 *
 * Modifications for RMCS by Zihan Qin, 2026:
 *   - Changed namespace
 *   - Adjusted defaults
 *   - Formatting changes
...
template <typename Values = double, typename Weight = unsigned>
struct Centroid {
    Values mean;
    Weight weight;
...
class TDigest {
    ...
    struct TDigestImpl {
        ...
        explicit TDigestImpl(size_t size)
            : total_weight(0) {
            values.reserve(size);
        }
        ...
        insert_result insert(Values value, Weight weight) {
            ...
            total_weight += weight;
            return (
                values.size() != values.capacity() ? insert_result::OK
...
public:
    ...
    void insert(Values value, Weight weight) {
        if (!weight)
            throw std::invalid_argument("TDigest weight must be positive.");

        const auto insert_result = buffer.insert(value, weight);
        min_val = std::min(min_val, value);
        max_val = std::max(max_val, value);

        if (insert_result == TDigestImpl::insert_result::NEED_COMPRESS) {
            merge();
        }
    }
...
void TDigest<Values, Weight>::swap(TDigest<Values, Weight>& other) {
    const bool this_active_one = (active == &one);
    const bool other_active_one = (other.active == &other.one);
    ...
    active = other_active_one ? &one : &two;
    other.active = this_active_one ? &other.one : &other.two;
}
...
void TDigest<Values, Weight>::insert(const TDigest<Values, Weight>& src) {
    ...
    auto insert_fn = [this](const auto& val) {
        if (buffer.insert(val) == TDigestImpl::insert_result::NEED_COMPRESS) {
            merge();
        }
    };
...
void TDigest<Values, Weight>::merge() {
    ...
    // buffer.values is pre-seeded with the current digest after each merge, so the
    // presence of centroids here does not mean there are new samples to merge.
    if (buffer.total_weight == 0) {
        return;
    }
...
double TDigest<Values, Weight>::quantile(double p) const {
    ...
    const double total_weight = static_cast<double>(active->total_weight);
    const double index = (p / 100.0) * total_weight;
    ...
    const double first_half_weight = static_cast<double>(first.weight) / 2.0;
    if (first.weight > 1 && first_half_weight > 1.0 && index < first_half_weight) {
        return (lerp(min_val, first.mean, (index - 1.0) / (first_half_weight - 1.0)));
    }
    ...
    const double last_half_weight = static_cast<double>(last.weight) / 2.0;
    ...
    auto quantile_fn = [index, &weight_so_far,
                        &quantile](const centroid_t& left, const centroid_t& right) {
        const double delta_weight =
            (static_cast<double>(left.weight) + static_cast<double>(right.weight)) / 2.0;
        if (weight_so_far + delta_weight > index) {
            const double lower = index - weight_so_far;
            const double upper = weight_so_far + delta_weight - index;
...
double TDigest<Values, Weight>::cumulative_distribution(Values x) const {
    ...
    if (x < active->values.front().mean) {
        ...
        if (first.mean - min_val > 0) {
            ...
            const double first_half_weight = static_cast<double>(first.weight) / 2.0;
            return (
                (1.0 + (x - min_val) / (first.mean - min_val) * (first_half_weight - 1.0))
                / total_weight);
        }
        return (0);
    }
    if (x > active->values.back().mean) {
        ...
        if (max_val - last.mean > 0) {
            ...
            const double last_half_weight = static_cast<double>(last.weight) / 2.0;
            const double dq =
                (1.0 + (max_val - x) / (max_val - last.mean) * (last_half_weight - 1.0))
                / total_weight;
            return (1.0 - dq);
        }
        return (1.0);
    ...
    auto cdf_fn = [x, &weight_so_far, &cdf, total_weight](
        ...
        if (left.mean <= x && x < right.mean) {
            ...
            const double delta_weight =
                (static_cast<double>(right.weight) + static_cast<double>(left.weight)) / 2.0;
            const double base = weight_so_far + static_cast<double>(left.weight) / 2.0;
        ...
        weight_so_far += static_cast<double>(left.weight);
        return (false);
    };
