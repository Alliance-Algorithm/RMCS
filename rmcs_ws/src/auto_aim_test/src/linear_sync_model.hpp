...
public:
    ...
    auto update(const double camera_timestamp_sec, const double board_timestamp_sec) noexcept
        ...
        const auto denominator = lambda_ + regressor.dot(projected_covariance);
        ...
        covariance_ = (covariance_ - gain * regressor.transpose() * covariance_) / lambda_;
        covariance_ = 0.5 * (covariance_ + covariance_.transpose());
