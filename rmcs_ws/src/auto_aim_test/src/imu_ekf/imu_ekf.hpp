...
public:
    ...
    struct Config
    {
        ...
        Vec3 gyro_zero_offset = Vec3(-0.005450708363333335, -0.0008284202383333334, -0.0006914497383333334);
    ...
    bool initializeFromAccel(const Vec3 &accel_mps2, const double initial_yaw = 0.0)
    {
        ...
        Vec3 init_euler;
        init_euler << initial_yaw,
            std::asin(-latest_normalized_accel_(0)),
    ...
    bool step(const Vec3 &gyro_rad_per_sec, const std::optional<Vec3> &accel_mps2, const double dt)
    {
        if (!initialized_)
        {
    ...
    bool process(const std::optional<TimedSample> &gyro_sample,
        const std::optional<TimedSample> &accel_sample,
        const double now_timestamp,
        const double initial_yaw = 0.0)
    {
        ...
        if (gyro_sample.has_value())
        {
            ...
            latest_gyro_original_ = gyro_sample->valid ? calibrateGyro(gyro_sample->value) : latest_valid_gyro_;
        ...
        if (accel_sample.has_value())
        {
            ...
            if (accel_sample->valid)
            {
                ...
                const std::optional<Vec3> normalized_accel = normalizeVector(latest_accel_original_);
                if (normalized_accel.has_value())
                {
                    latest_normalized_accel_ = *normalized_accel;
        ...
        {
            // Match the firmware path: accel-only update is ignored until a gyro timestamp is available.
        }
        else if (gyro_update_available && accel_update_available)
        {
    ...
    [[nodiscard]] double lastChiSquareLoss() const noexcept
    {
        return last_chi_square_loss_;
    }

    [[nodiscard]] bool accelUpdateAccepted() const noexcept
    {
        return last_accel_update_accepted_;
    }

    [[nodiscard]] Vec4 quaternion() const noexcept
    {
        return published_quaternion_;
    }

    [[nodiscard]] Vec3 eulerYawPitchRoll() const noexcept
    {
        return published_euler_;
    }

    [[nodiscard]] Mat3 rotationMatrix() const noexcept
    {
        return published_rotation_;
    }

    [[nodiscard]] Vec4 axisAngle() const noexcept
    {
        return published_axis_angle_;
    }

    [[nodiscard]] Vec3 accelBody() const noexcept
    {
        return published_accel_body_;
    }

    [[nodiscard]] Vec3 gyroBody() const noexcept
    {
        return published_gyro_body_;
    }

    [[nodiscard]] Vec3 accelOdom() const noexcept
    {
        return published_accel_odom_;
    }

    [[nodiscard]] Vec3 gyroOdom() const noexcept
    {
        return published_gyro_odom_;
    }

    [[nodiscard]] Vec3 originalAccel() const noexcept
    {
        return latest_accel_original_;
    }

    [[nodiscard]] Vec3 originalGyro() const noexcept
    {
        return latest_gyro_original_;
    }

    [[nodiscard]] const Mat4 &covariance() const noexcept
    {
        return covariance_;
    }

    [[nodiscard]] const Mat4 &priorCovariance() const noexcept
    {
        return covariance_prior_;
    }
...
private:
    ...
    [[nodiscard]] PropagationResult propagateQuaternion(const Vec4 &state, const Vec3 &gyro_rad_per_sec, const double dt) const
    {
        ...
        const double inverse_norm = (std::isfinite(norm) && norm > config_.epsilon) ? 1.0 / norm : 1.0;

        PropagationResult result;
        result.orthogonalization = inverse_norm * (Mat4::Identity() - inverse_norm * inverse_norm * quaternion_tmp * quaternion_tmp.transpose());
    ...
    {
        return orthogonalization_tmp_ * (Mat4::Identity() + 0.5 * dt * omegaMatrix(gyro_rad_per_sec));
    }
    ...
    [[nodiscard]] static Vec3 measurementModel(const Vec4 &state)
    {
        ...
        result(2) = state(0) * state(0) - state(1) * state(1) - state(2) * state(2) + state(3) * state(3);
        return result;
    }
    ...
    [[nodiscard]] static Vec3 eulerFromQuaternion(const Vec4 &quaternion)
    {
        ...
        result(0) = std::atan2(2.0 * (quaternion(0) * quaternion(3) + quaternion(1) * quaternion(2)), 1.0 - 2.0 * (quaternion(2) * quaternion(2) + quaternion(3) * quaternion(3)));

        const double sin_pitch = 2.0 * (quaternion(0) * quaternion(2) - quaternion(3) * quaternion(1));
        if (std::fabs(sin_pitch) >= 1.0)
        {
            result(1) = sin_pitch > 0.0 ? std::numbers::pi_v<double> / 2.0 : -std::numbers::pi_v<double> / 2.0;
        }
        else
        {
        ...
        result(2) = std::atan2(2.0 * (quaternion(0) * quaternion(1) + quaternion(2) * quaternion(3)), 1.0 - 2.0 * (quaternion(1) * quaternion(1) + quaternion(2) * quaternion(2)));
        return result;
    }
    ...
    [[nodiscard]] static Vec4 axisAngleFromQuaternion(const Vec4 &quaternion, const double epsilon)
    {
    ...
    bool doPredict(const Vec3 &gyro_rad_per_sec, const double dt)
    {
        ...
        const PropagationResult propagation = propagateQuaternion(current_state, gyro_rad_per_sec, dt);
    ...
    bool doUpdateFromNormalizedAccel(const Vec3 &normalized_accel)
    {
        ...
        const Mat3 innovation_covariance = jacobian_h_x * covariance_prior_ * jacobian_h_x.transpose() + config_.measurement_noise;
        const Mat3 innovation_covariance_inverse = innovation_covariance.inverse();

        last_chi_square_loss_ = (innovation.transpose() * innovation_covariance_inverse * innovation)(0, 0);
        if (!std::isfinite(last_chi_square_loss_) || last_chi_square_loss_ > config_.accel_chi_square_threshold)
        {
            last_accel_update_accepted_ = false;
            return false;
        }

        const Eigen::Matrix<double, 4, 3> kalman_gain = covariance_prior_ * jacobian_h_x.transpose() * innovation_covariance_inverse;
    ...
    {
        const Vec3 gravity_body = published_rotation_.transpose() * Vec3(0.0, 0.0, -config_.gravity_acceleration);
