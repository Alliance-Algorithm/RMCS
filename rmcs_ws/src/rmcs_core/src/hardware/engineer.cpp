...
private:
    ...
    public:
        ...
        explicit ArmBoard(Engineer& engineer, EngineerCommand& engineer_command, int usb_pid)
            ...
            , Joint({engineer, engineer_command, "/arm/Joint4"},
...

        {
            ...
            Joint[0].configure(
                LKMotorConfig{LKMotorType::MS5005}.reverse().set_encoder_zero_point(
                    static_cast<uint16_t>(engineer.get_parameter("joint4_zero_point").as_int())));
           joint[2].configure(
                device::DMMotorConfig{device::DMMotorType::DM4310}.reverse().set_encoder_zero_point(
                    static_cast<int>(engineer.get_parameter("joint3_zero_point").as_int())));
            joint[1].configure(
                device::DMMotorConfig{device::DMMotorType::DM4310}.reverse().set_encoder_zero_point(
                    static_cast<uint16_t>(engineer.get_parameter("joint2_zero_point").as_int())));
            joint[0].configure(
                device::DMMotorConfig{device::DMMotorType::DM6006}.reverse().set_encoder_zero_point(
        ...
        void arm_command_update() {
            ...
            if (counter % 2 == 0) {
                ...
                transmit_buffer_.add_can1_transmission(
                    0x003, std::bit_cast<uint64_t>(std::bit_cast<uint64_t>(uint64_t{command_})));

                command_ = joint[1].generate_torque_command();
                transmit_buffer_.add_can1_transmission(
                    0x000, std::bit_cast<uint64_t>(std::bit_cast<uint64_t>(uint64_t{command_})));
    ...
    private:
        ...
        device::DMMotor joint[3];
        device::LKMotor Joint[2];
        device::Dr16 dr16_;
