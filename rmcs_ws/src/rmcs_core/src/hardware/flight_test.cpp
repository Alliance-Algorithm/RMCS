...
public:
    FlightTest()
        ...
        , command_component_(
              create_partner_component<FlightTestCommand>(
    ...
    void update() override {
        ...
        // RCLCPP_INFO(get_logger(), "[steer calibration] New right front offset: %f", 0.1);
    }
...
private:
    ...
    class FlightTestCommand : public rmcs_executor::Component {
    public:
        explicit FlightTestCommand(FlightTest& flightTest)
            : flightTest(flightTest) {}

        void update() override { flightTest.command_update(); }
    ...
    public:
        friend class FlightTest;
        explicit TopBoard(
            FlightTest& flightTest, FlightTestCommand& flightTest_command, int usb_pid = -1)
            : CBoard(usb_pid)
            , tf_(flightTest.tf_)
            , bmi088_(1000, 0.2, 0.0)
            , dr16_(flightTest)

            , gimbal_pitch_motor_(flightTest, flightTest_command, "/gimbal/pitch")
            , gimbal_top_yaw_motor_(flightTest, flightTest_command, "/gimbal/top_yaw")
            , gimbal_left_friction_(flightTest, flightTest_command, "/gimbal/left_friction")
            , gimbal_right_friction_(flightTest, flightTest_command, "/gimbal/right_friction")
            , gimbal_bullet_feeder_(flightTest, flightTest_command, "/gimbal/bullet_feeder")

            , transmit_buffer_(*this, 32)
            , event_thread_([this]() { handle_events(); }) {

            flightTest.register_output("/referee/serial", referee_serial_);
            ...
            gimbal_pitch_motor_.configure(
                device::LkMotor::Config{device::LkMotor::Type::MG4010E_I10}.set_encoder_zero_point(
                    static_cast<int>(flightTest.get_parameter("pitch_motor_zero_point").as_int())));

            gimbal_top_yaw_motor_.configure(
                device::LkMotor::Config{device::LkMotor::Type::MG4010E_I10}.set_encoder_zero_point(
                    static_cast<int>(
                        flightTest.get_parameter("top_yaw_motor_zero_point").as_int())));
            ...
            flightTest.register_output("/gimbal/yaw/velocity_imu", gimbal_yaw_velocity_bmi088_);
            flightTest.register_output("/gimbal/pitch/velocity_imu", gimbal_pitch_velocity_bmi088_);
        ...
        void command_update() {
            ...
            // transmit_buffer_.add_can1_transmission(0x141,
            // gimbal_top_yaw_motor_.generate_command());

            // transmit_buffer_.add_can2_transmission(0x142,
            // gimbal_pitch_motor_.generate_command());

            transmit_buffer_.add_can1_transmission(0x200, gimbal_bullet_feeder_.generate_command());
    ...
    //         SteeringInfantry& steeringInfantry, SteeringInfantryCommand&
    //         steeringInfantry_command, int usb_pid = -1) : librmcs::client::CBoard(usb_pid) ,
    //         imu_(1000, 0.2, 0.0) , tf_(steeringInfantry.tf_) , dr16_(steeringInfantry) ,
    ...
    //             batch_commands[0] = chassis_wheel_motors_[0].generate_command();
    //             batch_commands[1] = chassis_wheel_motors_[1].generate_command();
    //             batch_commands[2] = 0;
    //             batch_commands[3] = 0;
    ...
    //             batch_commands_1[1] = chassis_wheel_motors_[3].generate_command();
    //             batch_commands_1[2] = 0;
    //             batch_commands_1[3] = chassis_wheel_motors_[2].generate_command();
    ...
    //             batch_commands[0] = chassis_steer_motors_[0].generate_command();
    //             batch_commands[1] = chassis_steer_motors_[1].generate_command();
    //             batch_commands[2] = 0;
    //             batch_commands[3] = 0;
    ...
    //             batch_commands_1[1] = chassis_steer_motors_[2].generate_command();
    //             batch_commands_1[0] = chassis_steer_motors_[3].generate_command();
    ...
    std::shared_ptr<FlightTestCommand> command_component_;
...
PLUGINLIB_EXPORT_CLASS(rmcs_core::hardware::FlightTest, rmcs_executor::Component)