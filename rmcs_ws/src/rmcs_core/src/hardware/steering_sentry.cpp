...
class SteeringSentry : public rmcs_executor::Component, public rclcpp::Node {
public:
  ...
  SteeringSentry()
      : Node{get_component_name(),
        ...
        logger_(get_logger()),
        sentry_command_(create_partner_component<SentryCommand>(
            get_component_name() + "_command", *this)),
        upper_construction_data{
            this,
            logger_,
            std::bit_cast<Flag>(static_cast<uint32_t>(
                get_parameter("upper_board_flag").as_int())),
        ...
        lower_construction_data{
            this,
            logger_,
            std::bit_cast<Flag>(static_cast<uint32_t>(
                get_parameter("lower_board_flag").as_int())),
    ...
    chassis_steer_motors_[0].configure(
        DjiMotorConfig{DjiMotorType::GM6020}
            .set_encoder_zero_point(static_cast<int>(
                get_parameter("left_front_zero_point").as_int()))
            .enable_multi_turn_angle());
    chassis_steer_motors_[1].configure(
        DjiMotorConfig{DjiMotorType::GM6020}
            .set_encoder_zero_point(static_cast<int>(
                get_parameter("left_back_zero_point").as_int()))
            .enable_multi_turn_angle());
    chassis_steer_motors_[2].configure(
        DjiMotorConfig{DjiMotorType::GM6020}
            .set_encoder_zero_point(static_cast<int>(
                get_parameter("right_back_zero_point").as_int()))
            .enable_multi_turn_angle());
    chassis_steer_motors_[3].configure(
        DjiMotorConfig{DjiMotorType::GM6020}
            .set_encoder_zero_point(static_cast<int>(
                get_parameter("right_front_zero_point").as_int()))
            .enable_multi_turn_angle());
  ...
  void update() override {
    if (!connected_) {
      upper_forwarder =
          (ForwarderFactory::Create<SteeringSentry>(upper_construction_data));
      lower_forwarder =
          (ForwarderFactory::Create<SteeringSentry>(lower_construction_data));
      connected_ = true;
    }
    update_motors();
    update_imu();
...
private:
  ...
  void lower_board_can1_receive_callback(uint32_t can_id, uint64_t can_data,
                                         bool is_extended_can_id,
                                         bool is_remote_transmission,
                                         uint8_t can_data_length) {
    if (is_extended_can_id || is_remote_transmission || can_data_length < 8)
  ...
  void lower_board_can2_receive_callback(uint32_t can_id, uint64_t can_data,
                                         bool is_extended_can_id,
                                         bool is_remote_transmission,
                                         uint8_t can_data_length) {
    if (is_extended_can_id || is_remote_transmission || can_data_length < 8)
  ...
  void upper_board_accelerometer_receive_callback(int16_t x, int16_t y,
                                                  int16_t z) {
    accelerometer_data_.store({x, y, z}, std::memory_order::relaxed);
  }

  void lower_board_accelerometer_receive_callback(int16_t x, int16_t y,
                                                  int16_t z) {
    accelerometer_data_.store({x, y, z}, std::memory_order::relaxed);
  }
...
public:
  ...
  void command_update() {
    ...
    lower_forwarder->add_can2_transmission(
    ...
    lower_forwarder->add_can1_transmission(
        0x200, std::bit_cast<uint64_t>(can_commands));
    // RCLCPP_INFO(logger_, "%d", can_commands[0]);
    lower_forwarder->trigger_transmission();
  }
...
private:
  ...
  public:
    explicit SentryCommand(SteeringSentry &infantry) : sentry_(infantry) {}
  ...
  device::DjiMotor gimbal_pitch_motor_{*this, *sentry_command_,
                                       "/gimbal/pitch"};

  device::DjiMotor gimbal_left_friction_{*this, *sentry_command_,
                                         "/gimbal/left_friction"};
  device::DjiMotor gimbal_right_friction_{*this, *sentry_command_,
                                          "/gimbal/right_friction"};
  device::DjiMotor gimbal_bullet_feeder_{*this, *sentry_command_,
  ...
  ForwarderConstructData<SteeringSentry> upper_construction_data;
  ForwarderConstructData<SteeringSentry> lower_construction_data;
...
PLUGINLIB_EXPORT_CLASS(rmcs_core::hardware::SteeringSentry,
