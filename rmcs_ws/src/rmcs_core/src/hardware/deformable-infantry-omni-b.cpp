...
private:
    ...
    auto status_service_callback(const std::shared_ptr<std_srvs::srv::Trigger::Response>& response)
        ...
        text("Chassis Status");
        text("- left front: {}", bottom_board_->chassis_joint_motors_[0].last_raw_angle());
        text("- left back: {}", bottom_board_->chassis_joint_motors_[1].last_raw_angle());
        text("- right back: {}", bottom_board_->chassis_joint_motors_[2].last_raw_angle());
        text("- right front: {}", bottom_board_->chassis_joint_motors_[3].last_raw_angle());
