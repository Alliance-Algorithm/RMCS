        ...
        for (auto& endpoint : endpoints_) {
            if (!endpoint.received) {
                if (endpoint.warn_cooldown == 0) {
                    ...
                    endpoint.warn_cooldown = kWarnCooldown;
                } else {
                    --endpoint.warn_cooldown;
                }
            } else {
                endpoint.warn_cooldown = 0;
            }
...
private:
    ...
    static constexpr std::size_t kWarnCooldown{10};
...
public:
    ...
    void command_update() {
        armboard_.command_update();
        encoderboard_.command_update();
        leftboard_.command_update();
        rightboard_.command_update();
    }
...
private:
    void gimbal_calibrate_subscription_callback(std_msgs::msg::Int32::UniquePtr) {
        RCLCPP_INFO(
            ...
            armboard_.joint[0].get_angle(), encoderboard_.joint2_encoder.get_angle(),
        ...
        RCLCPP_INFO(
            ...
            armboard_.joint[0].get_raw_angle(), encoderboard_.joint2_encoder.get_raw_angle(),
        ...
        void can_receive_callback(const Spec::Can& can, const View::Can& data) override {
            ...
            if (can == Spec::kCans.kCan2) {
                ...
                if (data.can_id == 0x143) {
                    ...
                    //  RCLCPP_INFO(this->get_logger(), "image raw %d  ,angle
                    //  %f",image_pitch.get_raw_angle(),image_pitch.get_angle());
    ...
    public:
        ...
        explicit EncoderBoard(
            ...
            const std::string_view& serial_filter = {})
        ...
        std::array<ReceiveMissingWatchdog::Endpoint, 1> receive_watchdog_entries_{
            {
             ...
             //  {ReceiveMissingWatchdog::EndpointType::kUart, "uart1", "/image_transmission"},
            }
        };
        ReceiveMissingWatchdog receive_watchdog_;
        std::unique_ptr<librmcs::board::CBoard> board_;
    ...
    protected:
        void can_receive_callback(const Spec::Can& can, const View::Can& data) override {
            ...
            if (can == Spec::kCans.kCan0) {
                ...
                // RCLCPP_INFO(this->get_logger(),"%x",data.can_id);
                receive_watchdog_.record_can("can2", data.can_id);
    ...
    private:
        void uart_receive_callback(const Spec::Uart& uart, const View::Uart& data) override {
            if (uart == Spec::kUarts.kUart1) {
                ...
                ring_buff.emplace_back_n(
                    [uart_data](std::byte* storage) mutable noexcept { *storage = *uart_data++; },
                    data.uart_data.size());
                while (ring_buff.peek_front() && ring_buff.readable() >= 16) {
                    std::byte rx_data[16];
                    std::byte* rx_ptr = rx_data;
                    ring_buff.pop_front_n(
                        [&rx_ptr](std::byte storage) noexcept { *rx_ptr++ = storage; }, 1);
                    if (rx_data[0] != std::byte{0x57}) {
                        continue;
                    }
                    ring_buff.pop_front_n(
                        [&rx_ptr](std::byte storage) noexcept { *rx_ptr++ = storage; }, 15);
                    tof.store_status(rx_data, 16);
                }
            }
        }

        rmcs_utility::RingBuffer<std::byte> ring_buff{16};
        device::Tof tof;
        device::DjiMotor Steering_motors[2];
        device::DjiMotor Wheel_motors[2];
        device::DjiMotor Omni_Motors;
