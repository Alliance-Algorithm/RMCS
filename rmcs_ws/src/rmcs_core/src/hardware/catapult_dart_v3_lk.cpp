...
//         // LK4005 lifting motors: multi-turn angle mode for absolute position control
...
//             "/limiting/calibrate", rclcpp::QoS{0},
...
//             .can_data = lifting_left_motor_.generate_command().as_bytes(),
//         });
//         board.can1_transmit({
//             .can_id   = LK_LIFTING_RIGHT_ID,
//             .can_data = lifting_right_motor_.generate_command().as_bytes(),
//         });
...
//                 auto command_buffer =
//                     limiting_servo_.generate_runtime_command(uart_data_length);
...
//                 RCLCPP_INFO(
//                     logger_, "[ForceSensor CAN1:0x302] raw(%zu): %s",
//                     data.can_data.size(),
...
//         RCLCPP_DEBUG(logger_, "UART2 received: len=%zu [%s]", length,
...
//     static constexpr uint32_t LK_LIFTING_RIGHT_ID = 0x142;
...
//             RCLCPP_INFO(logger_, "UART2 Pub: (length=%zu)[ %s ]", command_length,
