...
public:
    ...
    MecanumHero(const MecanumHero&) = delete;
    MecanumHero& operator=(const MecanumHero&) = delete;
    MecanumHero(MecanumHero&&) = delete;
    MecanumHero& operator=(MecanumHero&&) = delete;

    ~MecanumHero() override = default;
...
private:
    ...
    public:
        explicit HeroCommand(MecanumHero& hero)
            : hero_(hero) {}
    ...
    public:
        ...
        void command_update() {
            ...
            builder.can2_transmit({
                .can_id = 0x141,
                .can_data = gimbal_player_viewer_motor_
                                .generate_angle_command(gimbal_player_viewer_motor_.control_angle())
                                .as_bytes(),
            });
        }
            ...
            try {
                ...
                while (!stop_token.stop_requested()) {
                    if (external_imu_.store_status<uint8_t>(serial) && fps_counter.count()) {
                        const bool available = fps_counter.fps() > 350.0;
                        if (!available)
...
PLUGINLIB_EXPORT_CLASS(rmcs_core::hardware::MecanumHero, rmcs_executor::Component)
