...
// const UpStairsEvent go_to_Lift_And_Initial{UpStairsEventId::go_to_Lift_And_Initial, {}};
...
//     hardware::device::Trajectory<device::TrajectoryType::JOINT>& lift_and_initial;
...
//         hardware::device::Trajectory<device::TrajectoryType::JOINT>& lift_and_initial_,
//         hardware::device::Trajectory<device::TrajectoryType::JOINT>& initial_again_,
//         std::array<double, 6>& result_buf_)
//         : initial(initial_)
//         , press(press_)
//         , lift(lift_)
//         , lift_and_initial(lift_and_initial_)
...
//         if (event.id == UpStairsEventId::go_to_Lift_And_Initial) {
...
//     void enter(UpStairsContext& ctx, const EventArgs&) override { ctx.count = 0; }
//     std::optional<StepSubState>
//         handleEvent(const Event<UpStairsEventId>& event, UpStairsContext& ctx) override {
//         if (event.id == UpStairsEventId::go_to_Lift_And_Initial) {
//             return StepSubState::LiftAndInitial;
...
//             } else {
//                 return StepSubState::Lift; // for TwoStep
//                 // return StepSubState::LiftAndInitial;  //for OneStep
//             }
//         }
...
// class LiftAndInitialState final : public IState<StepSubState, UpStairsEventId, UpStairsContext> {
// public:
//     StepSubState getStateID() const override { return StepSubState::LiftAndInitial; }

//     void enter(UpStairsContext& ctx, const EventArgs&) override {
//         ctx.lift_and_initial.reset();
//         ctx.lift_and_initial.set_start_point(
//             std::array<double, 6>{
//                 **ctx.theta_lf, **ctx.theta_lb, **ctx.theta_rb, **ctx.theta_rf, 0.0, 0.0});
//         // ctx.lift_and_initial.set_total_step(static_cast<double>(ctx.calculate_steps(ctx.k[2],
//         // ctx.b[2])));
//         ctx.lift_and_initial.set_total_step(ctx.calculate_steps(ctx.k[3], ctx.b[3]));
//     }

//     std::optional<StepSubState>
//         handleEvent(const Event<UpStairsEventId>& event, UpStairsContext& ctx) override {
//         if (event.id == UpStairsEventId::tick) {
//             if (!ctx.lift_and_initial.get_complete()) {
//                 ctx.result = ctx.lift_and_initial.trajectory();
//                 return StepSubState::LiftAndInitial;
//             } else {
//                 return StepSubState::PressAgain; // for OneStep
//             }
//         }
//         return std::nullopt;
//     }
...
//         registerInnerState<LiftAndInitialState>();
...
//               up_stairs_leg_lift_and_initial, up_stairs_leg_initial_again, result)
