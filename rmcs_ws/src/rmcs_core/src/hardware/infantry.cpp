...
class Infantry : public rmcs_executor::Component,
                 public rclcpp::Node,
  ...
  Infantry()
      : Node{get_component_name(),
        ...
        forwarder::CBoard{
            ...
            get_logger()},
        logger_(get_logger()),
        infantry_command_(create_partner_component<InfantryCommand>(
...
private:
  void update_motors() {
    ...
    for (auto &motor : chassis_wheel_motors_)
      motor.update();
    tf_->set_state<BaseLink, LeftFrontWheelLink>(
        chassis_wheel_motors_[0].get_angle());
    tf_->set_state<BaseLink, RightFrontWheelLink>(
        chassis_wheel_motors_[1].get_angle());
    tf_->set_state<BaseLink, RightBackWheelLink>(
        chassis_wheel_motors_[2].get_angle());
    tf_->set_state<BaseLink, LeftBackWheelLink>(
        chassis_wheel_motors_[3].get_angle());
...
private:
  ...
  public:
    explicit InfantryCommand(Infantry &infantry) : infantry_(infantry) {}
  ...
  device::DjiMotor gimbal_pitch_motor_{*this, *infantry_command_,
                                       "/gimbal/pitch"};

  device::DjiMotor gimbal_left_friction_{*this, *infantry_command_,
                                         "/gimbal/left_friction"};
  device::DjiMotor gimbal_right_friction_{*this, *infantry_command_,
                                          "/gimbal/right_friction"};
  device::DjiMotor gimbal_bullet_feeder_{*this, *infantry_command_,
