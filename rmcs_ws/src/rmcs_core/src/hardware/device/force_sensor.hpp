...
public:
    explicit ForceSensor(rmcs_executor::Component& status_component) {
        status_component.register_output("/force_sensor/channel_1/weight", weight_ch1_, int32_t{0});
        status_component.register_output("/force_sensor/channel_2/weight", weight_ch2_, int32_t{0});
    }

    void store_status(std::span<const std::byte> can_data) {
        ForceSensorStatus status{};
        std::memcpy(&status, can_data.data(), std::min(can_data.size(), sizeof(ForceSensorStatus)));
    ...
    void update_status() {
        ...
        *weight_ch1_ = std::bit_cast<int32_t>(ch1_weight);
        *weight_ch2_ = std::bit_cast<int32_t>(ch2_weight);
    }
...
private:
    ...
    rmcs_executor::Component::OutputInterface<int32_t> weight_ch1_;
    rmcs_executor::Component::OutputInterface<int32_t> weight_ch2_;
};
