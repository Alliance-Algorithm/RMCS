...
struct DjiMotorConfig {
  ...
  DjiMotorConfig &set_encoder_zero_point(int value) {
    return encoder_zero_point = value, *this;
  }
  DjiMotorConfig &set_reduction_ratio(double value) {
    return reduction_ratio = value, *this;
  }
  DjiMotorConfig &reverse() { return reversed = true, *this; }
  DjiMotorConfig &enable_multi_turn_angle() {
    return multi_turn_angle_enabled = true, *this;
  }
};

class DjiMotor {
public:
  DjiMotor(Component &status_component, Component &command_component,
    ...
    raw_current_to_torque_coefficient_ = torque_to_raw_current_coefficient_ =
        0.0;

    status_component.register_output(name_prefix + "/reduction_ratio",
                                     reduction_ratio_, 0.0);
    status_component.register_output(name_prefix + "/max_torque", max_torque_,
                                     0.0);
    ...
    command_component.register_input(name_prefix + "/control_torque",
                                     control_torque_);
  }
  ...
  void configure(const DjiMotorConfig &config) {
    ...
    velocity_to_raw_velocity_coefficient_ =
        1 / raw_velocity_to_velocity_coefficient_;
    ...
    raw_current_to_torque_coefficient_ = sign * config.reduction_ratio *
                                         torque_constant / raw_current_max *
                                         current_max;
  ...
  void update() {
    auto feedback = std::bit_cast<DjiMotorFeedback>(
        can_data_.load(std::memory_order::relaxed));
  ...
  uint16_t generate_command() const {
    ...
    double current =
        std::round((*motor_)->torque_to_raw_current_coefficient_ * torque);
...
private:
  ...
  double raw_velocity_to_velocity_coefficient_,
      velocity_to_raw_velocity_coefficient_;
