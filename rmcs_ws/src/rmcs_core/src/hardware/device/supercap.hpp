...
public:
  explicit Supercap(Component &status_component, Component &command_component) {
    ...
    status_component.register_output("/chassis/supercap/voltage",
                                     supercap_voltage_, 0.0);
    status_component.register_output("/chassis/supercap/enabled",
                                     supercap_enabled_, false);

    command_component.register_input("/chassis/supercap/control_enable",
                                     supercap_control_enabled_);
    command_component.register_input("/chassis/supercap/control_power_limit",
                                     supercap_control_power_limit_);
  }

  void store_status(uint64_t can_data) {
    can_data_.store(std::bit_cast<SupercapStatus>(can_data),
  ...
  uint16_t generate_command() const {
    ...
    if (std::isnan(power_limit))
      ...
      command.power_limit =
          static_cast<uint8_t>(std::clamp(power_limit, 0.0, 255.0));

    return std::bit_cast<uint16_t>(command);
  }

private:
  static constexpr double uint_to_double(std::unsigned_integral auto value,
