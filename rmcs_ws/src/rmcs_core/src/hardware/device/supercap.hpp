...
public:
    explicit Supercap(Component& status_component, Component& command_component) {
        ...
        command_component.register_input("/chassis/supercap/control_enable", supercap_control_enabled_);
...
private:
    static constexpr double uint_to_double(std::unsigned_integral auto value, double min, double max) {
        double span = max - min;
        double offset = min;
