...
/// 达妙科技 DM-J8009-2EC 减速电机驱动（CAN@1Mbps, 标准帧, 速度模式）
///
/// 协议依据（权威来源）：
...
///
/// 速度控制帧（帧 ID = 电机 CAN ID + 0x200）：
///   D[0..3] v_des，float32 小端（低位在前），单位 rad/s；D[4..7] 填 0
///   使用前须在调试助手将 Control Mode 设为 VEL 并写入；发送控制帧不会切换模式。
///
...
///
/// 角度环在控制器侧闭环（速度模式下发 v_des），本驱动只做定标、限幅与打包。
class DmMotor {
public:
    ...
    /// 速度模式：v_des [rad/s]，按 reversed 方向映射；V_MAX 同时用作软件后备限幅
    CanPacket8 generate_velocity_command(double v_des) const {
        if (!std::isfinite(v_des) || v_des == 0.0)
            return pack_velocity(0.0f);
        const double sign = reversed_ ? -1.0 : 1.0;
        const double v_motor = std::clamp(sign * v_des, -velocity_max_, velocity_max_);
        return pack_velocity(static_cast<float>(v_motor));
    }
    ...
    /// 速度命令帧 ID == 电机 CAN ID + 0x200
    std::uint32_t velocity_send_id() const noexcept { return 0x200u + id_; }
    ...
    /// 速度帧：float32 小端（低位在前），后 4 字节填 0
    static CanPacket8 pack_velocity(float velocity) {
        const auto bits = std::bit_cast<std::uint32_t>(velocity);
        std::array<std::byte, 8> bytes{};
        bytes[0] = static_cast<std::byte>(bits & 0xFF);
        bytes[1] = static_cast<std::byte>((bits >> 8) & 0xFF);
        bytes[2] = static_cast<std::byte>((bits >> 16) & 0xFF);
        bytes[3] = static_cast<std::byte>((bits >> 24) & 0xFF);
