...
private:
    ...
    public:
        ...
        void command_update() {
            ...
            auto gimbal_bullet_feeder_command =
            ...
            if (can_transmission_mode) {
                ...
                builder.can1_transmit(
                    {.can_id = 0x1FF, .can_data = as_byte_span(gimbal_bullet_feeder_command)});
                ...
                steer_builder.can2_transmit(
                    {.can_id = 0x1FE, .can_data = as_byte_span(batch_commands)});
            } else {
                auto builder = start_transmit();
                builder.can1_transmit({.can_id = 0x200, .can_data = as_byte_span(batch_commands)});
                builder.can1_transmit(
                    {.can_id = 0x1FF, .can_data = as_byte_span(gimbal_bullet_feeder_command)});
                auto gimbal_yaw_command =
                    gimbal_yaw_motor_.generate_velocity_command(gimbal_yaw_motor_.control_velocity() - imu_.gy());
                builder.can2_transmit({.can_id = 0x142, .can_data = as_byte_span(gimbal_yaw_command)});
            }
