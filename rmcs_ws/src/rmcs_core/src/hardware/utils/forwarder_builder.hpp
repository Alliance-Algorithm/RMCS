...
public:
  virtual void can1_receive_callback(uint32_t, uint64_t, bool, bool,
                                     uint8_t) const = 0;
  virtual void can2_receive_callback(uint32_t, uint64_t, bool, bool,
                                     uint8_t) const = 0;
  ...
  virtual void accelerometer_receive_callback(int16_t, int16_t,
                                              int16_t) const = 0;
...
public:
  ForwarderConstructData(T *instance_object, const rclcpp::Logger &logger,
      ...
      : logger(logger), flag{forwarder_flag},
        ...
        accelerometer_receive_callback_{
            forwarder_accelerometer_receive_callback},
        gyroscope_receive_callback_{forwarder_gyroscope_receive_callback},
        instance_(instance_object) {}

  inline void can1_receive_callback(uint32_t a, uint64_t b, bool c, bool d,
                                    uint8_t e) const override {
    (instance_->*can1_receive_callback_)(a, b, c, d, e);
  }
  inline void can2_receive_callback(uint32_t a, uint64_t b, bool c, bool d,
                                    uint8_t e) const override {
    (instance_->*can2_receive_callback_)(a, b, c, d, e);
  }
  inline void uart1_receive_callback(const std::byte *a,
                                     uint8_t b) const override {
    (instance_->*uart1_receive_callback_)(a, b);
  }
  inline void dbus_receive_callback(const std::byte *a,
                                    uint8_t b) const override {
    (instance_->*dbus_receive_callback_)(a, b);
  }
  inline void accelerometer_receive_callback(int16_t a, int16_t b,
                                             int16_t c) const override {
    (instance_->*accelerometer_receive_callback_)(a, b, c);
  }
  inline void gyroscope_receive_callback(int16_t a, int16_t b,
                                         int16_t c) const override {
...
private:
  void (T::*const can1_receive_callback_)(uint32_t, uint64_t, bool, bool,
                                          uint8_t);
  void (T::*const can2_receive_callback_)(uint32_t, uint64_t, bool, bool,
                                          uint8_t);
...
class Forwarder {
public:
  virtual bool add_can1_transmission(uint32_t, uint64_t, bool = false,
                                     bool = false, uint8_t = 8) const noexcept {
    return true;
  }
  virtual bool add_can2_transmission(uint32_t, uint64_t, bool = false,
                                     bool = false, uint8_t = 8) const noexcept {
    return true;
  }
  virtual bool add_uart1_transmission(const std::byte *, uint8_t) const {
    return true;
  }

  virtual bool add_uart2_transmission(const std::byte *, uint8_t) const {
    return true;
  }

  virtual bool add_dbus_transmission(const std::byte *, uint8_t) const {
    return true;
  }
...
template <typename T> class ForwarderProduct : Forwarder {};
...
public:
  ...
  inline bool
  add_can1_transmission(uint32_t can_id, uint64_t can_data,
                        bool is_extended_can_id = false,
                        bool is_remote_transmission = false,
                        uint8_t can_data_length = 8) const noexcept override {
    return transmit_buffer_->add_can1_transmission(
        can_id, can_data, is_extended_can_id, is_remote_transmission,
        can_data_length);
  };
  inline bool
  add_can2_transmission(uint32_t can_id, uint64_t can_data,
                        bool is_extended_can_id = false,
                        bool is_remote_transmission = false,
                        uint8_t can_data_length = 8) const noexcept override {
    return transmit_buffer_->add_can2_transmission(
        can_id, can_data, is_extended_can_id, is_remote_transmission,
        can_data_length);
  };
  inline bool add_uart1_transmission(const std::byte *uart_data,
                                     uint8_t uart_data_length) const override {
    return transmit_buffer_->add_uart1_transmission(uart_data,
                                                    uart_data_length);
  }
  inline bool add_uart2_transmission(const std::byte *uart_data,
                                     uint8_t uart_data_length) const override {
    return transmit_buffer_->add_uart2_transmission(uart_data,
                                                    uart_data_length);
  }
  inline bool add_dbus_transmission(const std::byte *uart_data,
                                    uint8_t uart_data_length) const override {
...
public:
  ...
  ForwarderProduct(ForwarderConstructData<T> &data)
    ...
    RCLCPP_INFO(data.logger, "CBoard");
    transmit_buffer_ =
        (std::make_unique<forwarder::CBoard::TransmitBuffer>(*this, 16));
  }
...
concept IsForwarderConstructData =
    std::is_same_v<N, ForwarderConstructData<T>> ||
    std::is_same_v<N, ForwarderConstructData<T> &>;

class ForwarderFactory {
public:
  template <typename N>
  static std::unique_ptr<const Forwarder>
  Create(ForwarderConstructData<N> &data) {
    switch (data.flag.board_type) {

    case 1:
      return std::unique_ptr<const Forwarder>(static_cast<const Forwarder *>(
          std::move(std::make_unique<ForwarderProduct<forwarder::CBoard>>(data)
                        .release())));
      break;
    case 0:
    default:
