...
public:
    explicit ArmConfig()
        ...
        , joint{
              Joint(*this, "/arm/joint_1"), Joint(*this, "/arm/joint_2"),
    ...
    void update() override {
        ...
        msg.position        = {
            wrap(*joint_motor_angle[0]), wrap(*joint_motor_angle[1]), wrap(*joint_motor_angle[2]),
            wrap(*joint_motor_angle[3]), wrap(*joint_motor_angle[4]), 0.0};
        joint_states_pub->publish(msg);
    }

private:
    static constexpr std::size_t num_axis = 6;
    static constexpr std::size_t num_motor = 5;
