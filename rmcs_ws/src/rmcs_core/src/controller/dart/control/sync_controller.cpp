...
public:
    ...
    void update() override {
        ...
        *motor2_control_error_ = motor2_error + relative_error + error_integral_;
        if (log_enable_) {
            RCLCPP_INFO(
                get_logger(), "v1:%12lf,v2:%12lf,error:%12lf,intergral:%12lf", *motor1_measurement_,
                *motor2_measurement_, relative_error, error_integral_);
        }
    }
...
PLUGINLIB_EXPORT_CLASS(rmcs_core::controller::dart::SyncController, rmcs_executor::Component)