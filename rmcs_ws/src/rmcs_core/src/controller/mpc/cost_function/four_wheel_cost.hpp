...
struct FourWheelCost {
public:
  explicit FourWheelCost(
      const std::vector<double> &lambdas, double time_interval,
      std::shared_ptr<model::car::CarDynamicModelBase> &&car_model) {
  ...
  [[nodiscard]] double
  calculate_cost(const std::vector<double> &control_vector) const {
    car_model_->update(
    ...
    return (lambdas_ *
            (target_matrix_ - calc_matrix).cwiseAbs().rowwise().sum())(0, 0);
  }

  void set_parameters(double yaw_angle, const Eigen::Matrix3d &target_matrix,
                      const Eigen::Vector2d &linear_velocity,
                      const Eigen::Vector2d &position,
                      double angular_velocity) {
    target_matrix_ = target_matrix;
  ...
  bool operator()(const double *const x, double *residual) const {
    residual[0] = calculate_cost(std::vector<double>(x, x + 8));
    return true;
  }
...
private:
  ...
  Eigen::Matrix3d target_matrix_;
  Eigen::Vector2d current_velocity_;
  Eigen::Vector2d position_;
  double angular_velocity_;
  std::shared_ptr<model::car::CarDynamicModelBase> car_model_;
};
} // namespace rmcs_core::controller::mpc::utils