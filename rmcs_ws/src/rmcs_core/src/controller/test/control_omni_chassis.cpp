...
class TestOmniWheelController
    : public rmcs_executor::Component
    , public rclcpp::Node {
public:
    TestOmniWheelController()
        : Node(
              get_component_name(),
    ...
    void update() override {
        ...
        const auto chassis_velocity = calculate_chassis_velocity(wheel_velocities);//运动学正解算
        auto chassis_control_torque = calculate_chassis_control_torque(chassis_velocity);//底盘速度控制器
        const auto wheel_pid_torques =
            calculate_wheel_pid_torques(wheel_velocities, (chassis_control_velocity_->vector));//可能结合了从电机速度环加上底盘速度闭环?
    ...
    ChassisControlTorque calculate_chassis_control_torque(const Eigen::Vector3d& chassis_velocity) {
        ...
        Eigen::Vector3d err = chassis_control_velocity_->vector - chassis_velocity;//底盘速度期望与实际误差值
        Eigen::Vector2d translational_torque =
            (-std::numbers::sqrt2 / 4 * wheel_radius_) * mess_
            * translational_velocity_pid_calculator_.update(err.head<2>()); //？？涉及到速度逆解算
        result.torque.x() = translational_torque.norm(); //norm()函数意思为：求矢量绝对值
...
//上面利用pid速度开环输出了  底盘的  平动扭矩和转动扭矩
        Eigen::Vector2d translational_torque_direction;   //平动力矩单位方向向量
        ...
        result.lambda = {-x + y, -x - y};//底盘运动学矩阵逆映射，用于生成后面各轮的力矩
    ...
    Eigen::Vector4d calculate_wheel_pid_torques(
        ...
        wheel_control_velocity *= -1 / (std::numbers::sqrt2 * wheel_radius_);// x(-1)
...
PLUGINLIB_EXPORT_CLASS(rmcs_core::controller::test::TestOmniWheelController, rmcs_executor::Component)