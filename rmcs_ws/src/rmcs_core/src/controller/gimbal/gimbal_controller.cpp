...
public:
    GimbalController()
        ...
        register_output("/gimbal/shooting/fire_controller_", fire_controller_, false);
...
private:
    ...
    void update_auto_aim_fire_controller() {
        ...
        double cosValNew = current_dir.vector.dot(target_dir.vector)
    ...
    void update_manual_control_direction(PitchLink::DirectionVector& dir) {
        ...
        auto delta_yaw = Eigen::AngleAxisd { 0.006 * joystick_left_->y() + 5.0 * mouse_velocity_->y(),
                                             Eigen::Vector3d::UnitZ() };
        auto delta_pitch = Eigen::AngleAxisd { -0.006 * joystick_left_->x() - 5.0 * mouse_velocity_->x(),
    ...
    void update_auto_control_direction(PitchLink::DirectionVector& dir) {
        ...
        auto yaw_angle = std::clamp(delta_angle * yaw_time + auto_control_angle_.x() - last_yaw_angle_,
                                    -auto_rotate_speed_ * 1e-3,
                                    auto_rotate_speed_ * 1e-3)
                       + last_yaw_angle_;
        ...
        dir = fast_tf::cast<PitchLink>(OdomImu::DirectionVector { -Eigen::Vector3d::UnitY() }, *tf_);
    ...
    rclcpp::Subscription<geometry_msgs::msg::Pose2D>::SharedPtr auto_control_angle_sub_;
