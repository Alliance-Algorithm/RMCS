...
private:
    auto calculate_angle_error() -> FlightTwoAxisGimbalSolver::AngleError {
        ...
        // Until SetToLevel has converged, hold/auto-aim mode is forbidden so an
        // idle input at startup cannot latch a target before leveling finishes.
        if (!level_reached_) {
        ...
        if (auto_aim_control_direction_.ready() && (mouse_->right || switch_right == Switch::UP)
