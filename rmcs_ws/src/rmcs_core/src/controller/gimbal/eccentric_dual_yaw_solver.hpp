...
public:
    ...
    private:
        auto update(EccentricDualYawSolver& s) const -> Error override {
            ...
            const auto brl_imu = fast_tf::cast<OdomGimbalImu>(
            ...
            const Eigen::Vector3d brl = normalize(*brl_imu);
            ...
            const double barrel_pitch = std::atan2(-dir_.z(), std::hypot(dir_.x(), dir_.y()));
