...
private:
    ...
    AutoClimbControl update_auto_climb_align() {
        ...
        RCLCPP_INFO_THROTTLE(logger_, *get_clock(), 500, "ALIGN: err=%.3f", err);

        if (std::abs(err) < kAutoClimbAlignThreshold) {
    ...
    static constexpr double kAutoClimbAlignThreshold = 0.05;
