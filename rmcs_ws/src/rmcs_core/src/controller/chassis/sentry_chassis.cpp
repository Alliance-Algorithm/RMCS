...
public:
    ...
    inline void update_wheel_velocities(bool spinning) {
        motor_calculator_.update_power_max();
        ...
        if (spinning)
            ...
            for (int i = 0; i <= spin_try_times; i++) {
                if (motor_calculator_.calculate_wheel_velocity_and_angle(move, ratio * spinning_velocity))
                    return;
...
private:
    ...
    static constexpr double spinning_velocity = spinning_omega * chassis_radius * std::numbers::sqrt2 / 2;

    static constexpr int    spin_try_times = 5;
    static constexpr double spin_try_decrease = 1.0 / spin_try_times;

    double          null_bind_;
    Eigen::Vector2d auto_control_velocity_;

    rclcpp::Subscription<geometry_msgs::msg::Pose2D>::SharedPtr auto_control_velocity_sub_;

    rclcpp::Subscription<std_msgs::msg::Bool>::SharedPtr auto_control_spinning_sub_;
...
PLUGINLIB_EXPORT_CLASS(rmcs_core::controller::chassis::steering_wheel::SentryChassis,
