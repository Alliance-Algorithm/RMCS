...
public:
    ...
    void update() override {
        ...
        if (!std::isfinite(*chassis_radius_) || *chassis_radius_ <= 1e-6) {
            reset_all_controls();
            return;
        }
...
private:
    ...
    Eigen::Vector2d constrain_chassis_control_torque(
        ...
        const double d =
            lambda_1 * (w1 - w3 + 2 * k1_ * (t1 - t3)) + lambda_2 * (w2 - w4 + 2 * k1_ * (t2 - t4));
...
PLUGINLIB_EXPORT_CLASS(
    rmcs_core::controller::chassis::DeformableOmniWheelController, rmcs_executor::Component)
