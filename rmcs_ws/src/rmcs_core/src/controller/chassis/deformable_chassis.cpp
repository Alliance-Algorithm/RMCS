...
public:
    ...
    DeformableChassis()
        ...
        , spin_ratio_(
              std::clamp(get_parameter_or("spin_ratio", 0.6), 0.0, 1.0))
        , mode_mgr_(*this) {
        ...
        register_input(
            "/gimbal/yaw/control_angle_error", gimbal_yaw_angle_error_,
            false);
        register_input(
            "/chassis/manual_yaw_velocity_override",
            manual_yaw_velocity_override_, false);

        register_input("/chassis/imu/pitch", chassis_imu_pitch_, false);
        register_input("/chassis/imu/roll", chassis_imu_roll_, false);
        register_input(
            "/chassis/imu/pitch_rate", chassis_imu_pitch_rate_, false);
        register_input(
            "/chassis/imu/roll_rate", chassis_imu_roll_rate_, false);

        for (size_t i = 0; i < kJointCount; ++i) {
            register_input(
                fmt::format(
                    "/chassis/{}_joint/physical_angle", kJointName[i]),
            ...
            register_output(
                fmt::format(
                    "/chassis/{}_joint/control_angle_error",
                    kJointName[i]),
                joint_angle_error_[i], nan_);
        }

        register_output(
            "/gimbal/scope/control_torque", scope_motor_control_torque,
            nan_);

        register_output("/chassis/angle", chassis_angle_, nan_);
        register_output(
            "/chassis/control_angle", chassis_control_angle_, nan_);

        register_output("/chassis/control_mode", mode_);
        register_output(
            "/chassis/control_velocity", chassis_control_velocity_);
        register_output(
            "/chassis/ctrl_hold_active", ctrl_hold_active_, false);
        register_output(
            "/chassis/active_suspension/active",
    ...
    void before_updating() override {
        if (!gimbal_yaw_angle_.ready()) {
            ...
            RCLCPP_WARN(
                get_logger(),
                "Failed to fetch \"/gimbal/yaw/angle\". Set to 0.0.");
        }
        if (!gimbal_yaw_angle_error_.ready()) {
            gimbal_yaw_angle_error_.make_and_bind_directly(0.0);
            RCLCPP_WARN(
                get_logger(),
                "Failed to fetch \"/gimbal/yaw/control_angle_error\". "
                "Set to 0.0.");
        }
    ...
    void update() override {
        ...
        do {
            ...
            double rotary_knob =
                rotary_knob_.ready() ? *rotary_knob_ : 0.0;

            mode_mgr_.update(
                switch_left, switch_right, keyboard,
                rotary_knob, update_dt());

            const auto& mm = mode_mgr_.result();
            *mode_                = mm.mode;
            *ctrl_hold_active_    = mm.ctrl_hold_active;
            *active_suspension_active_ = mm.suspension_active;
...
private:
    ...
    double update_dt() const {
        if (update_rate_.ready() && std::isfinite(*update_rate_)
            && *update_rate_ > 1e-6)
    ...
    void update_velocity_control() {
        Eigen::Vector2d translational_velocity =
            update_translational_velocity_control();
        double angular_velocity = update_angular_velocity_control();
        chassis_control_velocity_->vector << translational_velocity,
            angular_velocity;
    }

    Eigen::Vector2d update_translational_velocity_control() {
        const auto keyboard = *keyboard_;
        Eigen::Vector2d keyboard_move{
    ...
    double update_angular_velocity_control() {
        ...
        case rmcs_msgs::ChassisMode::SPIN: {
            bool forward = mode_mgr_.result().spinning_forward;
            angular_velocity =
                spin_ratio_
                * (forward ? angular_velocity_max_
        ...
        case rmcs_msgs::ChassisMode::STEP_DOWN: {
            double err = calculate_unsigned_chassis_angle_error(
            ...
            angular_velocity =
                following_velocity_controller_.update(err);
        } break;
    ...
    double calculate_unsigned_chassis_angle_error(
        double& chassis_control_angle) {
    ...
    std::array<double, kJointCount> read_feedback_() const {
        ...
        for (size_t i = 0; i < kJointCount; ++i)
            if (joint_physical_angle_[i].ready()
                && std::isfinite(*joint_physical_angle_[i]))
    ...
    std::array<double, kJointCount> compute_target_angles_rad_() const {
        const auto& mm = mode_mgr_.result();
        std::array<double, kJointCount> targets_deg = mm.joint_target_deg;

        std::array<double, kJointCount> targets_rad;
        for (size_t i = 0; i < kJointCount; ++i)
            targets_rad[i] = deg_to_rad(targets_deg[i]);

        if (mm.suspension_active) {
            double base = mm.low_prone
                              ? mode_mgr_.active_suspension_min_angle_rad()
                              : deg_to_rad(mm.base_angle_deg);
            targets_rad.fill(base);
        }
    ...
    void run_joint_intent_pipeline_() {
        ...
        std::array<double, kJointCount> target_angles_rad =
            compute_target_angles_rad_();

        suspension_.calibrate(
            *chassis_imu_pitch_, *chassis_imu_roll_,
            mm.symmetric_target, update_dt());

        *scope_motor_control_torque = suspension_.scope_torque(
            mm.suspension_active,
            mm.mode == rmcs_msgs::ChassisMode::SPIN);

        auto corrections = suspension_.update(
            *chassis_imu_pitch_ - suspension_.pitch_offset(),
            *chassis_imu_roll_ - suspension_.roll_offset(),
            *chassis_imu_pitch_rate_, *chassis_imu_roll_rate_,
            mm.suspension_active, mm.low_prone,
            mode_mgr_.min_angle(), mode_mgr_.max_angle(),
            mm.base_angle_deg, mode_mgr_.correction_inverted(),
            joint_mgr_.angle_states(), update_dt());

        joint_mgr_.run_trajectory(
            target_angles_rad, mm.suspension_active, update_dt());
    ...
    void publish_joint_targets_(
        ...
        const DeformableChassisActiveSuspension::Corrections& corr) {

        double min_angle_rad =
            mode_mgr_.active_suspension_min_angle_rad();
        double max_angle_rad = mode_mgr_.max_angle_rad();
        ...
        for (size_t i = 0; i < kJointCount; ++i) {
            ...
            double target =
                joint_mgr_.angle_state(i) + corr.angle[i];
            *joint_target_angle_[i] = std::clamp(
                target, min_angle_rad, max_angle_rad);
            *joint_target_velocity_[i] =
                joint_mgr_.velocity_state(i) + corr.velocity[i];
            *joint_target_acceleration_[i] =
                joint_mgr_.acceleration_state(i)
                + corr.acceleration[i];
            *joint_angle_error_[i] =
                std::isfinite(feedback_angles[i])
    ...
    std::array<InputInterface<double>, kJointCount>
        joint_physical_angle_;
    std::array<OutputInterface<double>, kJointCount>
        joint_target_angle_;
    std::array<OutputInterface<double>, kJointCount>
        joint_target_velocity_;
    std::array<OutputInterface<double>, kJointCount>
        joint_target_acceleration_;
    std::array<OutputInterface<double>, kJointCount>
        joint_angle_error_;
    ...
    DeformableChassisModeManager mode_mgr_;
...
PLUGINLIB_EXPORT_CLASS(
    rmcs_core::controller::chassis::DeformableChassis,
