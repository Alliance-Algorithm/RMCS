...
public:
    ...
    void configure(rclcpp::Node& node) {
        target_vel_limit_ = std::max(
            deg_to_rad_(std::abs(node.get_parameter_or("target_physical_velocity_limit", 180.0))),
            1e-6);
        target_acc_limit_ = std::max(
            deg_to_rad_(
                std::abs(node.get_parameter_or("target_physical_acceleration_limit", 720.0))),
            1e-6);
    ...
    void run_trajectory(
        ...
        for (size_t i = 0; i < kJointCount; ++i) {
            ...
            double& angle_state        = target_angle_state_rad_[i];
            double& velocity_state     = target_velocity_state_rad_[i];
            double& acceleration_state = target_acceleration_state_rad_[i];
            double target              = target_angles_rad[i];

            double vel_limit = suspension_active ? suspension_vel_limit_ : target_vel_limit_;
            double acc_limit = suspension_active ? suspension_acc_limit_ : target_acc_limit_;

            if (!std::isfinite(target) || !std::isfinite(angle_state))
                continue;

            double position_error    = target - angle_state;
            double stopping_distance = velocity_state * velocity_state / (2.0 * acc_limit);
    ...
    double angle_state(size_t i) const { return target_angle_state_rad_[i]; }
    double velocity_state(size_t i) const { return target_velocity_state_rad_[i]; }
    double acceleration_state(size_t i) const { return target_acceleration_state_rad_[i]; }
    bool joint_active(size_t i) const { return joint_target_active_[i]; }

    std::array<double, kJointCount> angle_states() const { return target_angle_state_rad_; }
...
private:
    ...
    std::array<double, kJointCount> target_angle_state_rad_        = {0.0, 0.0, 0.0, 0.0};
    std::array<double, kJointCount> target_velocity_state_rad_     = {0.0, 0.0, 0.0, 0.0};
    std::array<double, kJointCount> target_acceleration_state_rad_ = {0.0, 0.0, 0.0, 0.0};
