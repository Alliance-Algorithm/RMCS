...
private:
    ...
    static auto calculate_K(const double (&wheel_velocities)[5]) -> std::pair<double, double> {
        ...
        double K_0 = std::pow(
            K_dec_, -std::abs(
                        ((wheel_velocities[0] + wheel_velocities[2]) * 0.075 / 0.7 / std::sqrt(2)
                         + wheel_velocities[4]))
                        / std::max(
                            std::max(
                                std::abs(wheel_velocities[4]),
                                std::abs(
                                    (wheel_velocities[0] + wheel_velocities[2]) * 0.075 / 0.7
                                    / std::sqrt(2))),
                            min_omega_));
        double K_1 = std::pow(
            K_dec_, -std::abs(
                        ((wheel_velocities[1] + wheel_velocities[3]) * 0.075 / 0.7 / std::sqrt(2)
                         + wheel_velocities[4]))
                        / std::max(
                            std::max(
                                std::abs(wheel_velocities[4]),
                                std::abs(
                                    (wheel_velocities[1] + wheel_velocities[3]) * 0.075 / 0.7
                                    / std::sqrt(2))),
                            min_omega_));
        if (K_0 < 0.85)
            K_0 += 0.15;
