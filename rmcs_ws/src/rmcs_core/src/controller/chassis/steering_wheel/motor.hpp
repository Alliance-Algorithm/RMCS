...
template <hardware::device::DjiMotorType>
struct MotorPowerParamFactory
{
...
public:
    ...
    double predict_power() {
        ...
        c = k2_ * std::pow(velocity, 2) + no_load_power_; // Constant term: iron loss and no-load power

        return a + b + c; // Unit: W
    }

    std::tuple<double, double, double>& get_param() { return torque_calculator_param_; };
...
protected:
    ...
    double                                    control_torque_unrestricted_;
    const std::tuple<double, double, double>& motor_power_param;

private:
    const std::tuple<double, double, double>& create_motor(hardware::device::DjiMotorType motor) {
        switch (motor) {
        case hardware::device::DjiMotorType::M3508:
            return MotorPowerParamFactory<hardware::device::DjiMotorType::M3508>::param;
        case hardware::device::DjiMotorType::GM6020:
            return MotorPowerParamFactory<hardware::device::DjiMotorType::GM6020>::param;
        default:
...
{
public:
    SteerMotor(DerivedFromBoth auto* component, const std::string& name, hardware::device::DjiMotorType motor)
        : MotorBase(motor)
        , steer_angle_pid_calculator_(component->get_parameter("steers_angle_kp").as_double(),
                                      component->get_parameter("steers_angle_ki").as_double(),
                                      component->get_parameter("steers_angle_kd").as_double())
        , steer_velocity_pid_calculator_(component->get_parameter("steers_vel_kp").as_double(),
        ...
        component->get_parameter("steers_angle_integral_min", steer_angle_pid_calculator_.integral_min);
        component->get_parameter("steers_angle_integral_max", steer_angle_pid_calculator_.integral_max);
        component->get_parameter("steers_angle_output_min", steer_angle_pid_calculator_.output_min);
        component->get_parameter("steers_angle_output_max", steer_angle_pid_calculator_.output_max);

        component->get_parameter("steers_vel_integral_min", steer_velocity_pid_calculator_.integral_min);
        component->get_parameter("steers_vel_integral_max", steer_velocity_pid_calculator_.integral_max);
        component->get_parameter("steers_vel_output_min", steer_velocity_pid_calculator_.output_min);
        component->get_parameter("steers_vel_output_max", steer_velocity_pid_calculator_.output_max);
    }

    void set_target_angle_error(double error) {
        steer_velocity_pid_calculator_.update(steer_angle_pid_calculator_.update(error) - *velocity_measure_);
    }
...
{
public:
    WheelMotor(DerivedFromBoth auto* component, const std::string& name, hardware::device::DjiMotorType motor)
        : MotorBase(motor)
        , wheel_velocity_pid_calculator_(component->get_parameter("wheels_vel_kp").as_double(),
        ...
        component->get_parameter("wheels_vel_integral_min", wheel_velocity_pid_calculator_.integral_min);
        component->get_parameter("wheels_vel_integral_max", wheel_velocity_pid_calculator_.integral_max);
        component->get_parameter("wheels_vel_output_min", wheel_velocity_pid_calculator_.output_min);
        component->get_parameter("wheels_vel_output_max", wheel_velocity_pid_calculator_.output_max);
    }
