...
class ChassisController : public rmcs_executor::Component, public rclcpp::Node {
public:
  ChassisController()
  ...
  void update() override {
    ...
    do {
      ...
      if (switch_left != Switch::DOWN) {
        ...
        } else if (last_switch_right_ == Switch::MIDDLE &&
                   switch_right == Switch::DOWN) {
          spinning_mode_ = !spinning_mode_;
        }
      }

      auto keyboard_move = Eigen::Vector2d{0.5 * (keyboard.w - keyboard.s),
                                           0.5 * (keyboard.a - keyboard.d)};
      update_wheel_velocities(Eigen::Rotation2Dd{*gimbal_yaw_angle_} *
  ...
  void update_wheel_velocities(Eigen::Vector2d move) {
    ...
    double right_oblique =
        velocity_limit * (-move.y() * cos_45 + move.x() * sin_45);
    double left_oblique =
        velocity_limit * (move.x() * cos_45 + move.y() * sin_45);

    double velocities[4] = {right_oblique, left_oblique, -right_oblique,
                            -left_oblique};
...
PLUGINLIB_EXPORT_CLASS(rmcs_core::controller::chassis::ChassisController,
