...
public:
    ...
    void set_visible(bool value) {
        ...
        for (auto& number : horizontal_center_scale_numbers_)
            number.set_visible(value);
        for (auto& scale : vertical_center_rangefinder_scales_)
            scale.set_visible(value);
        for (auto& scale : vertical_side_rangefinder_scales_)
            scale.set_visible(value);
        for (auto& number : vertical_side_scale_numbers_)
            number.set_visible(value);
    }

    void update_pitch_angle(double display_angle, bool precise_enabled) {
        pitch_angle_.set_value(display_angle);
...
private:
    ...
    constexpr static uint16_t x_side_center_ = x_center_ - 200;
