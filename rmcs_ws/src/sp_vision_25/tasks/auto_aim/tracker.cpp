...
std::list<Target> Tracker::track(
    ...
    std::optional<Color> enemy_color_override) {
    ...
    // 过滤掉非我方装甲板
    const Color filter_color = enemy_color_override.value_or(enemy_color_);
...
std::tuple<omniperception::DetectionResult, std::list<Target>> Tracker::track(
    ...
    std::chrono::steady_clock::time_point t, std::optional<Color> enemy_color_override) {
...
bool Tracker::set_target(std::list<Armor>& armors, std::chrono::steady_clock::time_point t) {
    ...
    if (is_balance) {
        ...
        Eigen::VectorXd P0_dig{
            {1, 64, 1, 64, 1, 81, 0.4, 100, 1e-4, 0, 0}
        };
        ...
        Eigen::VectorXd P0_dig{
            {1, 64, 1, 64, 1, 64, 0.4, 100, 1e-4, 0, 0}
        };
